...
local autosnippets = {
  ...
  s({ trig = "infsum" }, { t("\\sum\\limits_{"), i(1), t("=1}^{\\infty}"), i(2) }, default_opts),
...
for k, v in pairs(math_types) do
  local snip = postfix({ trig = "([%a%d{}]*)" .. k , regTrig = true, hidden = true }, { f(function (_, parent)
...
for a = 2, 8, 1 do
  ...
  table.insert(
    autosnippets,
    s("tr" .. tostring(a), nodes, default_opts)
    -- really slow
    -- { condition = function()
